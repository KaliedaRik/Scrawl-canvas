...
// TODO - need to add in additional actions: __focusAction__ (for 'onfocus' events) and __blurAction__ (for 'onblur' events) - these will help capture/control keyboard mediated tabbing through a web page. The idea is that when an &lt;a> element becomes focussed via keyboard tabbing (or other assistive technology methods) then we will be able to cascade the activity back into Scrawl-canvas so appropriate (visual) responses can be made to indicate the event has happened. 
